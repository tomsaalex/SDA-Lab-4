...
void testCeaMaiFrecventaValoare()
{
	MD m;
	assert(m.ceaMaiFrecventaValoare() == NULL_TVALOARE);
	
	m.adauga(1, 3);
	assert(m.ceaMaiFrecventaValoare() == 3);
	...
	assert(m.ceaMaiFrecventaValoare() == 4);
	...
	assert(m.ceaMaiFrecventaValoare() == 2);
}

void testAllExtins() {
	testCreeaza();
	testAdauga();
	testCeaMaiFrecventaValoare();
	testSterge();
    testIterator();
	testQuantity();
}
